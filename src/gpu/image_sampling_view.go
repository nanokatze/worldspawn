...
func newSamplingView(image *Image) SamplingView {
	...
	return SamplingView{handle: uint32(index)}
}

func (samplingView SamplingView) Destroy() {
	index := int(samplingView.handle)
	if index == 0 {
		return
	}

	// TODO: do extra checks here. Also allow destroying null views
