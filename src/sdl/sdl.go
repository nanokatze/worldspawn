...
func CreateWindow(props ...func(props PropertiesID) error) (*Window, error) {
	sdlProps, err := CreateProperties()
	if err != nil {
		return nil, err
	}
	defer sdlProps.Destroy()

	for _, f := range props {
		if err := f(sdlProps); err != nil {
			return nil, err
		}
	}

	return createWindowWithProperties(sdlProps)
}
